    ...
    const [payment, setPayment] = useState([
        {
            ...
            choose: false,
            icon: <GiSwipeCard className="icon" />,
        },
        {
        ...
        },
        {
            method: "Boleto",
            choose: false,
            icon: <RiBarcodeLine className="icon" />,
        },
    ]);
    ...
    async function buyProducts() {
        ...
        try {
            ...
            await axios.post(
                `${BASE_URL}/sells`,
                sell,
                config(user.token, user.refreshToken)
            );
            await axios.delete(
                `${BASE_URL}/carts/${location.state.userId}`,
                config(user.token, user.refreshToken)
            );
            ...
            setTimeout(() => {
                alert(
                    `Email de compra finalizada enviado para o email ${user.email}!`
                );
                navigate("/");
            }, 2500);
        } catch (err) {
            ...
            <PurchaseWrapper>
                {isFinalize ? (
                    ...
                    <>
                        ...
                        <div className="buy-button" onClick={buyProducts}>
                            {loading ? (
                                <ThreeCircles
                                    color="#fff"
                                    width="40px"
